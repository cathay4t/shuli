...
use crate::ieee80211::wnm::{
    BTM_REQUEST_ACTION, BTM_RESPONSE_ACTION, BTM_STATUS_ACCEPT,
    WNM_ACTION_CATEGORY, build_btm_response, parse_btm_request,
};
...
fn test_parse_btm_request_candidates() {
    ...
    let req = parse_btm_request(&body).expect("parse");
...
fn test_parse_btm_request_too_short() {
    assert!(parse_btm_request(&[1, 0]).is_none());
}

#[test]
fn test_build_btm_response() {
    let target = [0x02, 0x00, 0x00, 0x00, 0x02, 0x00];
    let frame = build_btm_response(7, BTM_STATUS_ACCEPT, target);
    assert_eq!(frame[0], WNM_ACTION_CATEGORY);
    assert_eq!(frame[1], BTM_RESPONSE_ACTION);
    assert_eq!(frame[2], 7); // dialog token echoed
    assert_eq!(frame[3], BTM_STATUS_ACCEPT);
...
fn test_btm_action_numbers() {
    ...
    assert_eq!(BTM_REQUEST_ACTION, 7);
    assert_eq!(BTM_RESPONSE_ACTION, 8);
    assert_eq!(WNM_ACTION_CATEGORY, 10);
}
