...
use super::{
    AuthAction, AuthMethod, EapAction, EapPacket, ErrorKind, FastReconnect,
...
use crate::{ETH_ALEN, crypto::handshake4::FourWayState};

/// Pick the one-shot retry path after a transient link loss.
///
/// `DEAUTH_LEAVING` is generated by cfg80211 when the host itself
/// leaves the network (e.g. during system suspend); the AP may be gone
/// or the client may have moved, so a scan is the safe first step. Any
/// other transient reason means the AP dropped us while it may still be
/// reachable, which matches wpa_supplicant's fast-reconnect-to-the-same
/// -BSS path.
fn classify_fast_reconnect(
    reason: Option<Ieee80211ReasonCode>,
    has_last_bss: bool,
) -> FastReconnect {
    match reason {
        Some(Ieee80211ReasonCode::DeauthLeaving) | None => {
            FastReconnect::ScanNow
        }
        Some(_) if has_last_bss => FastReconnect::SameBss,
        Some(_) => FastReconnect::ScanNow,
    }
}
...
impl WifiIface {
    ...
    /// transient.
    ///
    /// A transient disconnect of an established link also arms a
    /// one-shot fast retry: a locally generated disconnect scans
    /// immediately, an AP-initiated one tries the last BSS directly.
    pub(crate) async fn handle_ap_disconnect(
        ...
        if self.roam.ft_roam.is_some() {
            ...
            if fatal {
                ...
                let has_last_bss = self.link.bss_info.bssid != [0; ETH_ALEN]
                    && self.link.bss_info.freq_mhz != 0;
                let fast = classify_fast_reconnect(reason, has_last_bss);
                self.mark_link_lost(fast).await;
            }
    ...
    /// Clean up the kernel connection state, then enter the retry path
    /// with a one-shot fast reconnect action.
    pub(crate) async fn mark_link_lost(&mut self, fast: FastReconnect) {
        // For a socket-owned connection the kernel keeps its connection
        // state (wdev->connected) until userspace cleans up, and rejects
        // the next ASSOCIATE with -EALREADY otherwise. wpa_supplicant
        // sends CMD_DEAUTHENTICATE here; CMD_DISCONNECT clears the same
        // state.
        if let Err(e) = self.core.nl.disconnect().await {
            log::debug!("disconnect cleanup failed: {e}");
        }
        self.fast_reconnect = Some(fast);
        self.state = WifiState::Failed;
    }
...
mod tests {
    ...
    #[test]
    fn fast_reconnect_scans_on_local_disconnect() {
        assert_eq!(
            classify_fast_reconnect(
                Some(Ieee80211ReasonCode::DeauthLeaving),
                true
            ),
            FastReconnect::ScanNow
        );
        assert_eq!(classify_fast_reconnect(None, true), FastReconnect::ScanNow);
    }

    #[test]
    fn fast_reconnect_prefers_last_bss_for_ap_disconnect() {
        assert_eq!(
            classify_fast_reconnect(
                Some(Ieee80211ReasonCode::DisassocDueToInactivity),
                true
            ),
